...
import { ArrowUpRight, TrendingDown, Check, Star, ArrowRight, ShieldCheck, Activity, Zap, Clock, Users, ChevronDown, FileText, Download, Calendar, MapPin, Quote, Lightbulb } from 'lucide-react';
...
// --- Animations ---
export const FadeInView: React.FC<{ children: React.ReactNode, delay?: number, className?: string }> = ({ children, delay = 0, className = "" }) => {
  return (
    <motion.div
...
export const ProgramCapsule: React.FC<{ 
  ...
  isElite?: boolean, 
  isPopular?: boolean 
}> = ({ title, price, description, bestFor, duration, features, cta, isElite, isPopular }) => {
  
  return (
    <motion.div 
      whileHover={{ y: -8 }}
      className={`${capsuleBaseClass} flex flex-col p-6 md:p-8 w-full h-full min-w-[85vw] md:min-w-0 snap-center
        ${isElite 
...
// --- NEW COMPONENTS FOR DETAILED PAGES ---

// Curriculum Card - Detailed Day/Week View
export const CurriculumCard: React.FC<{
    ...
    icon?: React.ElementType;
    index: number;
}> = ({ range, title, goal, methods, why, icon: Icon, index }) => {
    return (
        <motion.div 
            initial={{ opacity: 0, x: 20 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ delay: index * 0.1, duration: 0.5 }}
            className="group relative bg-white border border-gray-100 rounded-[1.5rem] p-6 md:p-8 shadow-sm hover:shadow-[0_10px_30px_-5px_rgba(53,147,141,0.15)] transition-all duration-300 overflow-hidden"
        >
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary-teal/5 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity"></div>
            
            <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-shrink-0">
                    <div className="w-14 h-14 rounded-2xl bg-secondary-wellness/10 flex items-center justify-center text-secondary-wellness group-hover:bg-secondary-wellness group-hover:text-white transition-colors duration-300">
                ...
                <div className="flex-grow">
                    <div className="flex items-center gap-3 mb-2">
                        <span className="text-xs font-bold uppercase tracking-widest text-primary-teal bg-primary-teal/10 px-2 py-1 rounded-md">{range}</span>
                    </div>
                    
                    <h3 className="text-xl font-heading font-bold text-primary-navy mb-2">{title}</h3>
                    <p className="text-gray-600 italic mb-4 text-sm border-l-2 border-secondary-energy pl-3">"Goal: {goal}"</p>
                    
                    <div className="grid md:grid-cols-2 gap-x-4 gap-y-2 mb-4">
                        {methods.map((m, i) => (
                            <div key={i} className="flex items-start gap-2 text-sm text-gray-700">
                                <div className="w-1.5 h-1.5 rounded-full bg-primary-navy mt-1.5 flex-shrink-0"></div>
                                {m}
                            </div>
                        ))}
                    </div>
                    
                    <div className="bg-neutral-light p-4 rounded-xl flex items-start gap-3">
                        <Lightbulb className="w-5 h-5 text-secondary-energy flex-shrink-0 mt-0.5" />
                        <div>
                            <span className="text-xs font-bold uppercase text-gray-400 block mb-1">The Science</span>
                            <p className="text-sm text-primary-navy font-semibold">{why}</p>
                        </div>
                    </div>
                </div>
            </div>
...
// Impact Metric Card for ROI Section
export const ImpactMetricCard: React.FC<{
    title: string;
    description: string;
    result: string;
    icon: React.ElementType;
    ...
    return (
        <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ delay, duration: 0.5 }}
            className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 hover:bg-white/10 transition-colors"
        >
            <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-xl bg-secondary-energy/20 flex items-center justify-center text-secondary-energy border border-secondary-energy/20">
                    <Icon className="w-6 h-6" />
                </div>
            ...
            <div className="pt-4 border-t border-white/10 flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-secondary-wellness mt-1.5 flex-shrink-0"></div>
                <p className="text-secondary-wellness font-bold text-sm">{result}</p>
            </div>
        </motion.div>
    );
};
